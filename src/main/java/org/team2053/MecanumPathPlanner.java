...
public class MecanumPathPlanner
{
	...
	public void writeSmoothCSV(String fileName) {
		try {
			double time = 0.0;
			Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName), "utf-8"));
			for(double[] pt : this.smoothPath) {
				writer.write(time + "," + pt[0] + "," + pt[1] + "," + pt[2] + "\n");
				time = time + 0.02;
			}
			writer.close();
		} catch(IOException e) {
	...
	public void writeOriginalCSV(String fileName) {
		try {
			Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName), "utf-8"));
			for(double[] pt : this.origPath) {
				writer.write(pt[0] + "," + pt[1] + "," + pt[2] + "\n");
			}
			writer.close();
		} catch(IOException e) {
